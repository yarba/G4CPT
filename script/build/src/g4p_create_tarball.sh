...
GEANT4_LIB=${GEANT4_BASE}/geant4.${GEANT4_RELEASE}/lib
...
if [ ! -d ${TAR_DESTINATION}/build/g4.${GEANT4_RELEASE} ]; then
   mkdir -p ${TAR_DESTINATION}/build/g4.${GEANT4_RELEASE} && echo "... Creating, ${TAR_DESTINATION}/build/g4.${GEANT4_RELEASE} ..."
fi
# ---> PBS_TARBALL=${GEANT4_BASE}/geant4.${GEANT4_RELEASE}.${APPLICATION_NAME}.tar.gz
PBS_TARBALL=${TAR_DESTINATION}/build/g4.${GEANT4_RELEASE}/geant4.${GEANT4_RELEASE}.${APPLICATION_NAME}.tar.gz
...
sed "s%\${G4P_G4DIR}/lib%${TARGET_RAMDISK}/g4.${GEANT4_RELEASE}/geant4.${GEANT4_RELEASE}/lib%" \
     ${APPLICATION_DIR}/setenv.sh > ${APPLICATION_DIR}/setenv_pbs.sh

if [ x"${APPLICATION_NAME}" = x"cmsExp" ]; then
  sed -i "s%${APPLICATION_DIR}%${TARGET_RAMDISK}/g4.${GEANT4_RELEASE}/${APPLICATION_NAME}%" \
       ${APPLICATION_DIR}/setenv_pbs.sh 
fi

if [ x"${APPLICATION_NAME}" = x"cmsExp" ]; then
 tar --exclude=\.svn -czf ${PBS_TARBALL} \
 g4.${GEANT4_RELEASE}/geant4.${GEANT4_RELEASE}/lib \
 g4.${GEANT4_RELEASE}/*/bin/${APPLICATION_NAME} \
 g4.${GEANT4_RELEASE}/*/setenv_pbs.sh \
 g4.${GEANT4_RELEASE}/*/*.gdml \
 g4.${GEANT4_RELEASE}/*/*.mag.3_8T
elif [ x"${APPLICATION_NAME}" = x"lArTest" ]; then
 tar --exclude=\.svn -czf ${PBS_TARBALL} \
 g4.${GEANT4_RELEASE}/geant4.${GEANT4_RELEASE}/lib \
 g4.${GEANT4_RELEASE}/lArTest/bin/${APPLICATION_NAME} \
 g4.${GEANT4_RELEASE}/lArTest/setenv_pbs.sh \
 g4.${GEANT4_RELEASE}/lArTest/lArBox.gdml
else
 tar --exclude=\.svn -czf ${PBS_TARBALL} \
 g4.${GEANT4_RELEASE}/geant4.${GEANT4_RELEASE}/lib \
 g4.${GEANT4_RELEASE}/*/bin/${APPLICATION_NAME} \

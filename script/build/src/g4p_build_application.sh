...
if [ x"${APPLICATION_NAME}" = x"cmssw" ]; then
  GEANT4_BASE=${PROJECT_DIR}/build/g4.${GEANT4_RELEASE}_cmssw_${APPLICATION_RELEASE}
  APPLICATION_DIR=${GEANT4_BASE}/CMSSW_${APPLICATION_RELEASE}
fi
...
#  export DG4LIB_BUILD_GDML=1
  ...
  if [ x"${APPLICATION_NAME}" = x"SimplifiedCalo" -o 
       x"${APPLICATION_NAME}" = x"lArTest"]; then
...
#        -DXERCESC_ROOT_DIR=${XERCESC_DIR} \
  ...
  if [ x"${APPLICATION_NAME}" = x"cmsExp" -o x"${APPLICATION_NAME}" = x"cmsExpMT" ]; then
    echo "export CMSEXP_GDML=${APPLICATION_DIR}/cmsExp.gdml" \
          >> ${env_sh} 

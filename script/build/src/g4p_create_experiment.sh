...
tar_name=geant4.${GEANT4_RELEASE}.tar.gz
#-------------------------------------------------------------------------------
...
#  exp_env="${g4p_dir}/${APPLICATION_NAME}/setenv.sh"

elif [ x"${APPLICATION_NAME}" = x"cmssw" ]; then

  g4p_dir=${BLUEARC_DIR}/g4.${GEANT4_RELEASE}_cmssw_${APPLICATION_RELEASE}
  exp_exe="cmsRun"
  exp_cfg="run_sim_cmssw_cfg.py"
  exp_inp="events_zprime.root"
  exp_env="${g4p_dir}/CMSSW_${APPLICATION_RELEASE}/src/setup_csssw.sh"

  [ -f ${exp_env} ] && rm -rf ${exp_env}
  sed "s?G4P_CMSSW_SRC_DIR?${g4p_dir}/CMSSW_${APPLICATION_RELEASE}/src?" \
      g4p_setup_csssw.sh > ${exp_env}
else
  echo "... No g4p experiment for ${APPLICATION_NAME} ..." ; exit 1
fi

#------------------------------------------------------------------------------
# Check whether the application available on bluearc area
#------------------------------------------------------------------------------

#if [ ! -d ${g4p_dir} ]; then
#  echo "...The g4p application, ${APPLICATION_NAME} doesn't exist ..."; exit 1
#fi 
...
exp_dir=${BLUEARC_DIR}/pbs/g4p_${GEANT4_RELEASE}_${APPLICATION_NAME}_${EXP_NUM}
...
if [ x"${node_name}" == x"cmssrv140.fnal.gov" ]; then
  template_submit="${cfg_dir}/template.submit_tool"
#elif [ x"${node_name}" == x"grunt1" ]; then
elif [ x"${node_name}" == x"cluck.fnal.gov" ]; then
  template_submit="${cfg_dir}/template.submit_pbs"
elif [ x"${node_name}" == x"tevnfsg4" ]; then
  template_submit="${cfg_dir}/template.submit_pbs"
else
...
if [ -f ${template_submit} ]; then
  ...
  #loop over tools
  for tool in sprof igprof ; do

    if [ x"${node_name}" == x"cmssrv140.fnal.gov" ]; then
      template_tool="${cfg_dir}/template.run_${tool}"
#    elif [ x"${node_name}" == x"grunt1" ]; then
    elif [ x"${node_name}" == x"cluck.fnal.gov" ]; then
      template_tool="${cfg_dir}/template.pbs_${tool}"
    elif [ x"${node_name}" == x"tevnfsg4" ]; then
...
#        ln -s ${g4p_dir}/${APPLICATION_NAME}/gdml/cmsExp_calo.gdml \
#              ${work_dir}/cmsExp.gdml
      fi
    done
    # create files to submit pbs jobs (on the wilson)
    if [ -f ${cfg_dir}/template.wilson_submit_${tool} ]; then
      ...
      echo "Warning ${cfg_dir}/template.submit_wilson_${tool} does not exist!"
    fi
  done
else 

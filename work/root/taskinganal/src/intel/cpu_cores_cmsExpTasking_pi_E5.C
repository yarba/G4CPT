...
int cpu_cores_cmsExpTasking_pi_E5()
{
   gROOT->Reset();
...
//   int beam[nb] = {0,1,2,4,6,8,10,12};
   ...
   for(int i = 0 ; i < nb ; i++) {
     sprintf(gffilename,"./cpu_core_pi-_E5_t%d.dat",beam[i]);
...
// -->   double pbin[nt], epbin[nt];
   ...
   for(int i = 0 ; i < nt ; i++) {
     ...
     gf_mt_ratio_rms[i]  = 0.001*gf_mt_ratio_mean[i]*sqrt(a*a+b*b);
   }

   c2->cd();
   TPostScript *ps2 = new TPostScript("cpu_cores_cmsExpTasking_pi_E5.eps",113);
...
// -->   TLine* llow = new TLine(0.7, 1.0, 15, 1.0);
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"cpu_cores_cmsExpTasking_pi_E5.%s","png");

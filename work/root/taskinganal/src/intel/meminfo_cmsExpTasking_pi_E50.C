...
int meminfo_cmsExpTasking_pi_E50()
{
   gROOT->Reset();
...
//   int beam[nb] = {0,1,2,4,6,8,10,12};
   ...
   for(int i = 0 ; i < nb ; i++) {
     sprintf(gffilename,"./meminfo_pi-_E50_t%d.dat",beam[i]);
...
// -->   double pbin[nt], epbin[nt];
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("meminfo_cmsExpTasking_pi_E50.eps",113);
...
// -->   TH2F* hhf3 = new TH2F("hhf3","Memory (RSS-SHARED) - 50 GeV #pi^{-}", 1,0.7,15.,1,99.99,4000.);
   TH2F* hhf3 = new TH2F("hhf3","Memory (RSS-SHARED) - 50 GeV #pi^{-}", 1,0.7,20.,1,99.99,4000.);
...
// -->   TLine* llow = new TLine(1.0, gf_serial_mean[0], 12, 12*gf_serial_mean[0]);
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"meminfo_cmsExpTasking_pi_E50.%s","png");

...
int memeff_cmsExpTasking_pi_E50()
{
   gROOT->Reset();
...
// -->    double pbin[nt], epbin[nt];
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("memeff_cmsExpTasking_pi_E50.eps",113);
...
// -->   TH2F* hhf3 = new TH2F("hhf3","Memory Reduction - 50 GeV #pi^{-}", 1,0.7,15.,1,0.0,1.6);
   TH2F* hhf3 = new TH2F("hhf3","Memory Reduction - 50 GeV #pi^{-}", 1,0.7,20.,1,0.0,1.6);
...
// -->   TLine* ll = new TLine(0.7, 1.0, 15, 1.0);
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"memeff_cmsExpTasking_pi_E50.%s","png");

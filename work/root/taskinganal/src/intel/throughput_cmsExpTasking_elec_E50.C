...
int throughput_cmsExpTasking_elec_E50()
{
   gROOT->Reset();
...
// -->   double pbin[nt], epbin[nt];
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("throughput_cmsExpTasking_elec_E50.eps",113);
...
// -->   TLine* llow = new TLine(1.0, gf_total_mean[0], 12, 12.0*gf_total_mean[0]);
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"throughput_cmsExpTasking_elec_E50.%s","png");

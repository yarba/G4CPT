...
int throughput_cmsExpTasking_elec_E5()
{
   gROOT->Reset();
...
//   int beam[nb] = {0,1,2,4,6,8,10,12};
   ...
   for(int i = 0 ; i < nb ; i++) {
     sprintf(gffilename,"./scaling_e-_E5_t%d.dat",beam[i]);
   ...
   for(int i = 0 ; i < nb ; i++) {
     ...
     h_total[i] = new TH1F(hname,htitle, 10000,0.0,500.0);

     while ( !feof(gffile[i]) ) {
       fscanf(gffile[i],"%f",&total);
       if(i==0) h_total[i]->Fill(1024.0/total);
       else     h_total[i]->Fill(beam[i]*1024.0/total);
     }
...
// -->   double pbin[nt], epbin[nt];
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("throughput_cmsExpTasking_elec_E5.eps",113);
...
// -->   TH2F* hhf3 = new TH2F("hhf3","Event Throughput - 5 GeV e^{-}", 1,0.7,15.,1,1.0,120.);
   TH2F* hhf3 = new TH2F("hhf3","Event Throughput - 5 GeV e^{-}", 1,0.7,20.,1,1.0,200.);
...
// -->   TLine* llow = new TLine(1.0, gf_total_mean[0], 12, 12.0*gf_total_mean[0]);
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"throughput_cmsExpTasking_elec_E5.%s","png");

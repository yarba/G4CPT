...
int memeff_cmsExpTasking_elec_E5()
{
   gROOT->Reset();
...
// -->   double pbin[nt], epbin[nt];
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("memeff_cmsExpTasking_elec_E5.eps",113);
...
// -->   TLine* ll = new TLine(0.7, 1.0, 15, 1.0);
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"memeff_cmsExpTasking_elec_E5.%s","png");

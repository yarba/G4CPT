...
int throughput_cmsExpTasking_pi_E5()
{
   gROOT->Reset();
...
// -->   double pbin[nt], epbin[nt];
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("throughput_cmsExpTasking_pi_E5.eps",113);
...
// -->   TH2F* hhf3 = new TH2F("hhf3","Event Throughput - 5 GeV #pi^{-}", 1,0.7,15.,1,1.0,120.);
   TH2F* hhf3 = new TH2F("hhf3","Event Throughput - 5 GeV #pi^{-}", 1,0.7,20.,1,1.0,120.);
...
// -->   TLine* llow = new TLine(1.0, gf_total_mean[0], 12, 12.0*gf_total_mean[0]);
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"throughput_cmsExpTasking_pi_E5.%s","png");

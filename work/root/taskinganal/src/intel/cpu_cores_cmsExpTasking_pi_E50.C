...
int cpu_cores_cmsExpTasking_pi_E50()
{
   gROOT->Reset();

   TCanvas* c2 = new TCanvas("c2","c2",0,0,500,400);
   c2->SetTitle("Geant4 MT Performance: cmsExpTasking");
...
//   int beam[nb] = {0,1,2,4,6,8,10,12};
   ...
   for(int i = 0 ; i < nb ; i++) {
     sprintf(gffilename,"./cpu_core_pi-_E50_t%d.dat",beam[i]);
   ...
   for(int i = 0 ; i < nb ; i++) {
     ...
     std::cout << "pi E50 i-core Mean CPU and RMS " 
...
// -->   double pbin[nt], epbin[nt];
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("cpu_cores_cmsExpTasking_pi_E50.eps",113);
...
// -->   TH2F* hhf3 = new TH2F("hhf3","Speedup Efficiency - 50 GeV #pi^{-}", 1,0.7,15.,1,0.8,1.1499);
   TH2F* hhf3 = new TH2F("hhf3","Speedup Efficiency - 50 GeV #pi^{-}", 1,0.7,20.,1,0.8,1.1499);
...
// -->   TLine* llow = new TLine(0.7, 1.0, 15, 1.0);
   ...
   lg1->AddEntry(hgr3,"cmsExp/cmsExpTasking","PL");
   lg1->SetTextSize(0.04);
   lg1->Draw();

   char pngtitle[256];   
   sprintf(pngtitle,"cpu_cores_cmsExpTasking_pi_E50.%s","png");

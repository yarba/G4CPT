...
int meminfo_cmsExpTasking_elec_E50()
{
   gROOT->Reset();

   TCanvas* c2 = new TCanvas("c2","c2",0,0,500,400);
   c2->SetTitle("Geant4 Tasking Performance: cmsExpTasking");
...
//   int beam[nb] = {0,1,2,4,6,8,10,12};
   ...
   for(int i = 0 ; i < nb ; i++) {
     sprintf(gffilename,"./meminfo_e-_E50_t%d.dat",beam[i]);
...
// -->   double pbin[nt], epbin[nt];
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("meminfo_cmsExpTasking_elec_E50.eps",113);
...
// -->   TH2F* hhf3 = new TH2F("hhf3","Memory (RSS-SHARED) - 50 GeV e^{-}", 1,0.7,15.,1,99.99,4000.);
   TH2F* hhf3 = new TH2F("hhf3","Memory (RSS-SHARED) - 50 GeV e^{-}", 1,0.7,20.,1,99.99,4000.);
...
// -->   TLine* llow = new TLine(1.0, gf_serial_mean[0], 12, 12*gf_serial_mean[0]);
   TLine* llow = new TLine(1.0, gf_serial_mean[0], 20, 20*gf_serial_mean[0]);
   llow->SetLineColor(2);
   llow->SetLineStyle(2);
   llow->Draw();
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"meminfo_cmsExpTasking_elec_E50.%s","png");

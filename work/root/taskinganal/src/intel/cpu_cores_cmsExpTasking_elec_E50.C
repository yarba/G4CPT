...
int cpu_cores_cmsExpTasking_elec_E50()
{
   gROOT->Reset();
...
//   int beam[nb] = {0,1,2,4,6,8,10,12};
   ...
   for(int i = 0 ; i < nb ; i++) {
     sprintf(gffilename,"./cpu_core_e-_E50_t%d.dat",beam[i]);
   ...
   for(int i = 0 ; i < nb ; i++) {
     ...
     sprintf(htitle,"CPU/Core for %d tracks",beam[i]);
...
// -->    double pbin[nt], epbin[nt];
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("cpu_cores_cmsExpTasking_e-_E50.eps",113);
/*
   TH2F   *hhf2;
   TGraph *hgr2;
   TGraph *hgr3;
...
// -->   TLine* llow = new TLine(0.7, 1.0, 15, 1.0);
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"cpu_cores_cmsExpTasking_elec_E50.%s","png");

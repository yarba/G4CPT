...
//   int beam[nb] = {0,1,2,4,6,8,10,12};
   ...
   for(int i = 0 ; i < nb ; i++) {
     ...
     h_total[i] = new TH1F(hname,htitle, 10000,0.0,200.0);
...
// -->   double pbin[nt], epbin[nt];
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("throughput_cmsExpMT_pi_E5.eps",113);
...
// -->   TLine* llow = new TLine(1.0, gf_total_mean[0], 12, 12.0*gf_total_mean[0]);
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"throughput_cmsExpMT_pi_E5.%s","png");

...
int sthroughput_cmsExpMT_pi_E50()
{
   gROOT->Reset();
...
//   int beam[nb] = {0,1,2,4,8,16,24,32};
   ...
   for(int i = 0 ; i < nb ; i++) {
     sprintf(gffilename,"./scaling_pi-_E50_t%d.sdat",beam[i]);
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("sthroughput_cmsExpMT_pi_E50.eps",113);
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"sthroughput_cmsExpMT_pi_E50.%s","png");

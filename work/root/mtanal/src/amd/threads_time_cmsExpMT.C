...
int threads_time_cmsExpMT()
{
   ...
   for(int i = 0 ; i < nb ; i++) {
     sprintf(gffilename,"/home/syjun/pbs/scaling/%d/scaling_%d.dat",beam[i],beam[i]);
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("threads_time_cmsExpMT.eps",113);
   ...
   */
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"threads_time_cmsExpMT.%s","png");

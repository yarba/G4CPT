...
//   int beam[nb] = {0,1,2,4,8,16,24,32};
   ...
   for(int i = 0 ; i < nb ; i++) {
     ...
     std::cout << "pi E50 i-core Mean CPU and RMS " << beam[i] << " " << gf_total_mean[i] << " " << gf_total_rms[i] << std::endl;
     fclose(gffile[i]);
   }
   ...
   hhf3 = new TH2F("hhf3","Speedup Efficiency - 50 GeV #pi^{-}", 1,0.7,40.,1,0.8,1.1499);

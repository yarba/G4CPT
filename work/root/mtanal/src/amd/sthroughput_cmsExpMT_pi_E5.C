...
int sthroughput_cmsExpMT_pi_E5()
{
   gROOT->Reset();
...
//   int beam[nb] = {0,1,2,4,8,16,24,32};
   ...
   for(int i = 0 ; i < nb ; i++) {
     sprintf(gffilename,"./scaling_pi-_E5_t%d.sdat",beam[i]);
   ...
   c2->cd();
   TPostScript *ps2 = new TPostScript("sthroughput_cmsExpMT_pi_E5.eps",113);
   ...
   char pngtitle[256];   
   sprintf(pngtitle,"sthroughput_cmsExpMT_pi_E5.%s","png");
